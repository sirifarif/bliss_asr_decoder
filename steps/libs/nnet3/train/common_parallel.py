...
def get_average_nnet_model_parallel(dir, iter, nnets_list, run_opts, suffix,
                           get_raw_nnet_from_am=False):

    next_iter = iter + 1
    if get_raw_nnet_from_am:
        out_model = ("""- \| nnet3-am-copy --set-raw-nnet=-  \
                        {dir}/{iter}.mdl {dir}/{next_iter}.{suffix}.mdl""".format(
                            dir=dir, iter=iter,
                            next_iter=next_iter, suffix=suffix))
    else:
        out_model = "{dir}/{next_iter}.{suffix}.raw".format(
            dir=dir, next_iter=next_iter, suffix=suffix)

    common_lib.execute_command(
        """{command} {dir}/log/average.{iter}.{suffix}.log \
...
def get_best_nnet_model_parallel(dir, iter, best_model_index, run_opts, suffix,
                        get_raw_nnet_from_am=False):

    best_model = "{dir}/{next_iter}.{suffix}.{best_model_index}.raw".format(
        dir=dir,
        next_iter=iter + 1,
        best_model_index=best_model_index, suffix=suffix)

    if get_raw_nnet_from_am:
        out_model = ("""- \| nnet3-am-copy --set-raw-nnet=- \
                        {dir}/{iter}.mdl {dir}/{next_iter}.{suffix}.mdl""".format(
                            dir=dir, iter=iter, next_iter=iter + 1, suffix=suffix))
    else:
        out_model = "{dir}/{next_iter}.{suffix}.raw".format(dir=dir, suffix=suffix,
                                                   next_iter=iter + 1)

    common_lib.execute_command(
        """{command} {dir}/log/select.{iter}.{suffix}.log \
...
def get_average_nnet_model_weighted(dir, iter, nnets_list, weights, run_opts,
    ...
    common_lib.execute_command(
        """{command} {dir}/log/average.{iter}.log \
                nnet3-average --weights={weights} {nnets_list} \
                {out_model}""".format(command=run_opts.command,
                                      dir=dir,
                                      iter=iter,
                                      nnets_list=nnets_list,
